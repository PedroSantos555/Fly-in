...
from baseclasses import Hub, Set_Up
...
def weight(start_hub: Hub, end_hub: Hub, time: int) -> float:
    ...
    if end_hub.max_drones <= end_hub.reserve_timetable.get(time, 0):
        print("Caught")
        weight += waiting_time(end_hub.reserve_timetable,
    ...
    def schedule_drones(self) -> None:
        ...
        for drone in self.drones:
            ...
            for number, hub in enumerate(path):
                if not self.hubs[hub].reserve_timetable.get(number):
                    self.hubs[hub].reserve_timetable[number] = 1
                else:
                    self.hubs[hub].reserve_timetable[number] += 1
            print(distance)
