...
def weight(start_hub: Hub, end_hub: Hub, time: int) -> float:
    ...
    #if (time < 25):
        #print(end_hub.name, "TIME CHECKED: ", time, "\n", end_hub.reserve_timetable)
    ...
    def schedule_drones(self) -> None:
        ...
        for drone in self.drones:
            ...
            for number, hub in enumerate(path):
                ...
                drone.script[number + turns_wait] = (next_hub.x, next_hub.y)

                if next_hub.status == "restricted":
                    middle_pos = (current_hub.x + ((next_hub.x - current_hub.x) / 2),
                                  (current_hub.y + (next_hub.y - current_hub.y) / 2)) 
                    drone.script[number + turns_wait - 1] = middle_pos     
...
if __name__ == "__main__":
    ...
    #for conection in setup.connections:
    #    print(f"{conection.start}-{conection.end}: ", conection.reserve_timetable)
