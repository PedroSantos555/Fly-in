...
def weight(start_hub: Hub, end_hub: Hub, time: int) -> float:
    ...
    if (connection is not None and connection.max_links is not None
            and connection.max_links <= connection.reserve_timetable.get(
                time, 0)):
...
def get_start_end(hubs: Dict[str, Hub]) -> Tuple[Hub, Hub]:
    ...
    if start_hub is None or end_hub is None:
        raise ValueError("Setup must contain one start_hub and one end_hub")
...
def shortest_path_calc(hubs: Dict[str, Hub],
    ...
    # MAYBE I convert the path+timetble into a proper schedule drones
    # follow each turn
    # OR MAYBE - I SEARCH IN THE TIME DIMENSION AS WELL AS THE SPACE DIMENSION
    # AND GIVE THIS ALGO AN EXTRA DIMENSION

    distance: Dict[str, float] = {hub: float("inf") for hub in hubs}
    previous: Dict[str, Optional[str]] = {hub: None for hub in hubs}
    distance[start.name] = 0
    neighbors: List[str] = []
    prio_q: List[Tuple[float, str]] = []
        ...
        for drone in self.drones:
            ...
            for number, hub in enumerate(path):
                ...
                if number > 0:
                    ...
                    if conection is None:
                        raise ValueError(
                                        ...
                                        f"{current_hub.name} and "
                                        f"{next_hub.name} doesnt exist"
                                        )

                    if conection.max_links is None:
                        raise ValueError("Connection has no capacity")
                    while (
                        conection.max_links
                        <= conection.reserve_timetable.get(
                            number + turns_wait, 0)
                    ):

                        if not current_hub.reserve_timetable.get(
                                number + turns_wait):
                            current_hub.reserve_timetable[
                                number + turns_wait] = 1
                        else:
                            current_hub.reserve_timetable[
                                number + turns_wait] += 1
                    ...
                    if not conection.reserve_timetable.get(
                            number + turns_wait):
                        conection.reserve_timetable[
                            number + turns_wait] = 1
                    else:
                        conection.reserve_timetable[
                            number + turns_wait] += 1
                ...
                if next_hub.kind == "end_hub":
                    ...
                    drone.script[number + turns_wait] = (
                        next_hub.x, next_hub.y, next_hub.name
                    )
                    continue
                ...
                while next_hub.max_drones <= next_hub.reserve_timetable.get(
                        number + turns_wait, 0):
                    if not current_hub.reserve_timetable.get(
                            number + turns_wait):
                ...
                if not next_hub.reserve_timetable.get(
                        number + turns_wait):
                    next_hub.reserve_timetable[
                        number + turns_wait] = 1
                else:
                    next_hub.reserve_timetable[
                        number + turns_wait] += 1
...
def end_turn(setup: Set_Up) -> int:
    ...
    """
    _start, end = get_start_end(setup.hubs)
    endturn = 0
