import pygame
import sys
import pygame.locals as loc
from typing import Any, List
from fly_parser import parser_file
from baseclasses import Set_Up, Camera, Drone
from collections import Counter
from scheduler import Schedule, shortest_path_calc, end_turn


def execute_turn(drones: List[Drone], n_turn: int, instant: bool) -> None:
    """Apply and print one simulation turn.

    Args:
        drones: All drones.
        n_turn: Turn number to execute.
        instant: Teleport sprites instead of animating them.
    """

    # print("TURN", n_turn)
    for drone in drones:
        if drone.script.get(n_turn) is not None:
            action = drone.script.get(n_turn)
            if action is None:
                continue
            x, y, name = action
            if instant:
                drone.sprite.teleport(x, y)
            else:
                drone.sprite.move(x, y)
            print(f"D{drone.id}-{name} ", end="")
    print("\n")


class Display:
    """Pygame window that shows the network and the drones."""

    drone_sprites: List[Any] = []
    hub_sprites: List[Any] = []
    white = (255, 255, 255)
    black = (0, 0, 0)
    oblue = (0, 119, 190)
    mygrey = (180, 180, 180)
    setup = None

    def draw_connections(self, surface: Any) -> None:
        """Draw a line for every connection.

        Args:
            surface: Target pygame surface.
        """
        if self.setup is None:
            return

        for hub in self.setup.hubs.values():
            start = hub.sprite.rect.center
            for connection in hub.conects:
                end = self.setup.hubs[connection.end].sprite.rect.center

                pygame.draw.line(surface=surface,
                                 start_pos=start,
                                 end_pos=end,
                                 width=2,
                                 color=self.black)

    def start_display(self, setup: Set_Up,
                      disp_surface: pygame.Surface) -> None:
        """Open the window and run the main loop.

        Controls: M autoplay, N next turn, P connections, 1/2/3 zoom,
        arrow keys pan.

        Args:
            setup: Scheduled simulation setup.
        """

        camera = Camera()
        self.setup = setup
        show_connections = False
        FPS = 60
        pygame.font.init()
        FramePerSec = pygame.time.Clock()
        pygame.display.set_caption("Fly-in")
        disp_surface
        disp_surface.fill(self.oblue)
        fps = pygame.time.Clock()
        fps.tick(60)

        clock = pygame.time.Clock()
        turn = 1
        timer = 0.0
        running = True
        autoplay = True

        for hub in setup.hubs.values():
            hub.sprite.draw(disp_surface, camera)
            self.hub_sprites.append(hub.sprite)

        for drone in setup.drones:
            self.drone_sprites.append(drone.sprite)

        while (running):
            dt = clock.tick(60) / 1000

            if (autoplay):
                timer += dt
                if timer >= 1.5 and turn <= end_turn(self.setup):
                    execute_turn(self.setup.drones, turn, False)
                    timer -= 1.5
                    turn += 1

            for event in pygame.event.get():
                if event.type == loc.QUIT:
                    pygame.quit()
                    sys.exit()

                if event.type == pygame.KEYDOWN:

                    if event.key == pygame.K_m:
                        autoplay = not autoplay

                    if event.key == pygame.K_n:
                        execute_turn(setup.drones, turn, True)
                        turn += 1

                    if event.key == pygame.K_p:

                        show_connections = not show_connections

                    if event.key == pygame.K_1:
                        for sprite in self.hub_sprites:
                            if sprite.size != 0.5:
                                sprite.mini()

                        for drone in self.drone_sprites:
                            if drone.size != 0.5:
                                drone.mini()

                        camera.zoom = 0.5

                    if event.key == pygame.K_2:
                        for sprite in self.hub_sprites:
                            if sprite.size != 1:
                                sprite.medium()

                        for drone in self.drone_sprites:
                            if drone.size != 1:
                                drone.medium()

                        camera.zoom = 1.0

                    if event.key == pygame.K_3:
                        for sprite in self.hub_sprites:
                            if sprite.size != 2:
                                sprite.big()

                        for drone in self.drone_sprites:
                            if drone.size != 2:
                                drone.big()

                            camera.zoom = 2.0

            pressed_keys = pygame.key.get_pressed()

            if any([pressed_keys[loc.K_UP], pressed_keys[loc.K_DOWN],
                   pressed_keys[loc.K_RIGHT], pressed_keys[loc.K_LEFT]]):

                offset = [0, 0]

                if pressed_keys[loc.K_UP]:
                    offset[1] += 16
                if pressed_keys[loc.K_DOWN]:
                    offset[1] += -16
                if pressed_keys[loc.K_LEFT]:
                    offset[0] += 16
                if pressed_keys[loc.K_RIGHT]:
                    offset[0] += -16

                camera.move(-offset[0], -offset[1])

            disp_surface.fill(self.oblue)

            for sprite in self.hub_sprites:
                sprite.draw(disp_surface, camera)

            for sprite in self.drone_sprites:
                sprite.update(FPS)

            positions = Counter((drone.world_pos.x, drone.world_pos.y)
                                for drone in self.drone_sprites)

            for coords, count in positions.items():
                drone = next(d for d in self.drone_sprites
                             if d.world_pos.x == coords[0]
                             and d.world_pos.y == coords[1])

                drone.draw(disp_surface, camera)

                font = pygame.font.Font(None, int(36 * camera.zoom))
                if count > 1:
                    text = font.render(str(count), True,
                                       (255, 255, 255))
                    disp_surface.blit(text, drone.rect)

            if show_connections:
                self.draw_connections(disp_surface)

            font = pygame.font.Font(None, 72)
            text = font.render(f"TURN {turn}", True, (255, 255, 255))

            disp_surface.blit(text, (10, 10))

            pygame.display.update()
            FramePerSec.tick(FPS)


if __name__ == "__main__":

    pygame.init()
    surface = pygame.display.set_mode((1920, 1080))
    setup = None
    # setup = parser_file("01_the_impossible_dream.txt")

    # setup = parser_file("FLYIN.maps/maps/easy/01_linear_path.txt")
    # setup = parser_file("FLYIN.maps/maps/easy/02_simple_fork.txt")
    # setup = parser_file("FLYIN.maps/maps/easy/03_basic_capacity.txt")
    # setup = parser_file("FLYIN.maps/maps/medium/01_dead_end_trap.txt")
    # setup = parser_file("FLYIN.maps/maps/medium/02_circular_loop.txt")
    setup = parser_file(
        "FLYIN.maps/maps/hard/01_maze_nightmare.txt"
    )
    # setup = parser_file("FLYIN.maps/maps/hard/02_capacity_hell.txt")
    # setup = parser_file("FLYIN.maps/maps/medium/03_priority_puzzle.txt")

    for key, hub in setup.hubs.items():
        neighbor = list()
        print(hub.name)
        for c in hub.conects:
            conect = f"{c.start}-{c.end}"
            neighbor.append(conect)

        print("neighbors ", neighbor)
    dist, prev = shortest_path_calc(setup.hubs, setup.hubs["start"])

    schedule = Schedule(setup)
    schedule.schedule_drones()
    dsp = Display()
    dsp.start_display(setup=setup, disp_surface=surface)
